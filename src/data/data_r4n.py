...
class R4NGenerator:
    ...
    def save_results(self, compounds, filename=None, validate_pubchem=False, get_properties=False, get_halide_cas=False, verbose=True):
        ...
        if validate_pubchem:
            ...
            desc = f"PubChem Validation{properties_text}"
            ...
            if get_halide_cas:
                ...
                # 遍历每一行
                for idx, row in tqdm(df_with_cid.iterrows(), total=len(df_with_cid), desc="Querying Halide CAS"):
                    smiles = row['SMILES']
                    
                    # 查询各种卤化盐的 CAS
                    for halide in halides:
                        halide_smiles = _add_halide_to_smiles(smiles, halide)
                        cas = _get_cas_number(halide_smiles, verbose=False, salt_type='')
                        df.at[idx, f'{halide_names[halide]}_CAS'] = cas
                
                if verbose:
                    cas_found = sum((df[f'{halide_names[h]}_CAS'] != '').sum() for h in halides)
                    print(f"\nHalide CAS query complete! Found {cas_found} CAS numbers.")
...
def clean_r4n_data(csv_path: str) -> Optional[str]:
    ...
    结果会保存为原文件名追加 "_with_cid" 的 CSV, 并返回输出路径字符串。
