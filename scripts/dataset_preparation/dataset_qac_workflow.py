...
支持两种主要模式：
1. 生成 QAC 化合物（基础生成，无 PubChem 验证）
2. 分步 PubChem 验证流水线:
...
如果需要在其他代码中调用，请使用:
    from src import QACGenerator, clean_qac_dataset
...
def generate_qac_compounds():
    """生成 QAC 化合物的交互式流程（仅基础生成，不含验证）"""
    print("=" * 50)
    print("QAC Cation Generator")
    print("=" * 50)
    ...
    while True:
        try:
            ...
            if max_carbons < 4:
                print("Error: QAC needs at least 4 carbons.")
                continue
            if max_carbons > 30:
    ...
    # 检查是否已存在有效数据
    filename = str(QAC_DIR / f'dataset_qac_c{max_carbons}.csv')
    ...
    # 生成化合物
    generator = QACGenerator(max_carbons)
    ...
    print(f"\n[OK] {len(compounds)} QAC cations saved to: {filename}")
...
def run_pubchem_validation():
    ...
    # 根据模式确定默认文件
    default_file = QAC_DIR / "dataset_qac_c20.csv"
    if mode == 'c':
        default_file = QAC_DIR / "dataset_qac_c20_with_cid.csv"
    elif mode == '2':
        # 优先 cleaned，其次 with_cid
        cleaned = QAC_DIR / "dataset_qac_c20_cleaned.csv"
        default_file = cleaned if cleaned.exists() else QAC_DIR / "dataset_qac_c20_with_cid.csv"
    elif mode == '3':
        default_file = QAC_DIR / "dataset_qac_c20_with_props.csv"
    ...
    try:
        if mode == 'a':
            ...
            if check_data_integrity(cleaned_csv, verbose=True):
                ...
                cleaned_csv = clean_qac_dataset(cid_csv)
            # Step 2
            ...
            if do_clean == 'y':
                cleaned_csv = clean_qac_dataset(output_csv)
...
def main():
    ...
    while True:
        print("\n" + "=" * 50)
        print("QAC Dataset Workflow")
        print("=" * 50)
        print("Please select an option:")
        print("1. Generate QAC compounds (basic, no PubChem)")
