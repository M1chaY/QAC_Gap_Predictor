...
加载QM9预训练模型，使用R4N数据集进行微调。

策略：
1. 读取预训练模型和配置
2. 加载R4N数据集，选取指定数量样本用于训练
...
def finetune_on_r4n(
    ...
    try:
        ...
        # 保存微调后模型
        finetuned_path = MODEL_DIR / "r4n_finetuned.pt"
        torch.save({
...
def main():
    """主函数。"""
    print("=" * 60)
    print("R4N GNN Finetuning Script (5-Fold CV)")
    print("=" * 60)
    ...
    # 执行微调（使用102条数据进行3折交叉验证，50条作为测试集）
    success = finetune_on_r4n(
